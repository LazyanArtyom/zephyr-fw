...
int CmdStatus(const shell* shell, size_t argc, char** argv) {
    ...
    shell_print(shell, "Application status: %s",
                context.IsInitialized() ? "running" : "not initialized");
    return 0;
}
...
int CmdVersion(const shell* shell, size_t argc, char** argv) {
    ...
    shell_print(shell, "Boot mode: %s", app::GetBootMode());
    shell_print(shell, "Git commit: %s%s", app::GetGitCommit(),
                app::IsGitDirty() ? " (dirty)" : "");
    shell_print(shell, "Built: %s", app::GetBuildTimestamp());
    return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(app_subcommands,
