...
int main() {
    ...
    if (startup_healthy) {
        const platform::Status boot_confirm_status =
            services::health::HealthService::ConfirmBootIfHealthy();
        if (!boot_confirm_status.ok()) {
            LOG_ERR("Boot confirmation skipped: %s (%s)",
                    platform::ToString(boot_confirm_status.code()).c_str(),
                    boot_confirm_status.message().c_str());
        }
    } else {
        LOG_WRN("Boot confirmation skipped due to startup health failures");
    }
